// https://atcoder.jp/contests/tessoku-book/submissions/38861382
#include <algorithm>
#include <iostream>
#include <vector>
...
struct SegmentTree {
    ...
    void update(size_t index, Element value) {
        ...
        for(;;) {
            ...
            width = next_width;
            offset >>= 1;
            elements_.at(start + offset) = std::max(left, right);
        }
    }
    ...
    Element find_detail(size_t left, size_t right, size_t level) {
        ...
        return std::max(find_detail(left, range_center_prev, level - 1),
