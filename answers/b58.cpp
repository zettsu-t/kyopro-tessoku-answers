// https://atcoder.jp/contests/tessoku-book/submissions/38861531
#include <algorithm>
#include <iostream>
#include <vector>
...
struct SegmentTree {
    static constexpr Element inf = 1000000000000LL;
    SegmentTree(size_t size) {
        ...
        elements_ = std::vector<Element>(n_leaves * 2, inf);
        top_ = n_leaves * 2 - 2;
    ...
    Element get_element(size_t index) const {
...
void solve(std::istream& is, std::ostream& os) {
    ...
    segtree.update(0, 0);
    for(decltype(1) i{1}; i<n; ++i) {
        const auto current = positions.at(i);
        auto it_current = positions.begin() + i;
        auto it_left = std::lower_bound(positions.begin(), it_current, current - r);
        auto it_right = std::upper_bound(positions.begin(), it_current, current - l);

        if ((it_left != it_current) && ((current - *it_left) >= l) && (it_right != positions.begin())) {
            --it_right;
            if ((current - *it_right) <= r) {
                const auto left = it_left - positions.begin();
                const auto right = it_right - positions.begin();
                const auto value = segtree.find(left, right) + 1;
                segtree.update(i, value);
            }
        }
    }

    os << segtree.get_element(n-1) << "\n";
    return;
}
